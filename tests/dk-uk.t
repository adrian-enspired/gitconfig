...
git -C "$AT_TMP/dk" config at.ticket.prefix COM
write_commit dk m 'm' 'chore: master work'

at_out dk dk 12345 my-slug >/dev/null
is "$AT_RC" '0' 'dk succeeds'
is "$(git -C "$AT_TMP/dk" branch --show-current)" 'COM-12345.my-slug' 'the branch is named from the ticket'
is "$(config dk branch.COM-12345.my-slug.atbase)" "$(sha dk master)" 'the branch point is the default branch tip'
is "$(config dk branch.COM-12345.my-slug.atparent)" '' 'a branch off the default branch has no parent'
...
at_out dk dk COM-2 other >/dev/null
...
at_out uk uk COM-3 child >/dev/null
is "$AT_RC" '0' 'uk succeeds'
is "$(git -C "$AT_TMP/uk" branch --show-current)" 'COM-3.child' 'uk names the branch the same way'
is "$(config uk branch.COM-3.child.atparent)" 'parent' 'the branch it came from is recorded as the parent'
is "$(config uk branch.COM-3.child.atbase)" "$parent_tip" 'and the parent tip as the branch point'
...
at_out f5 uk COM-4 child >/dev/null
...
is "$(config f5 branch.COM-4.child.atbase)" "$(sha f5 parent)" 'the branch point is the parent tip, not the quicksave'
...
at_out nof5 uk --no-f5 COM-5 child >/dev/null
is "$(git -C "$AT_TMP/nof5" branch --show-current)" 'COM-5.child' '--no-f5 still creates the branch'
...
at_out flat uk COM-6 thing >/dev/null
is "$(config flat branch.COM-6.thing.atparent)" '' 'uk from the default branch records no parent'
